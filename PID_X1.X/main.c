...
//PID
void PID1(unsigned int sp);

//variables generales
unsigned int i=0;
//variables de velocidad
short statevel1=0,statevel2=0,statevel3=0;
unsigned int Vel1=0,Vel2=0,Vel3=0,Vel1_=0,Vel2_=0,Vel3_=0;
//Variables UART
...
//variables PID_1
signed int error=0;
float cv=0.0,cv_1=0.0,error_1=0.0,error_2=0.0;
float kp=1.6;//2.50;//5.5
float ki=34.58;//40.11;//100.0
float kd=0.0;
float Ts=0.015;
/**/
...
void PID1(unsigned int sp)
{
    ...
    /*
    //ZOH
    float M1_q0=kp+(kd/Ts);
    float M1_q1=(-kp+(ki*Ts-(2*kd/Ts)));
    float M1_q2=(kd/Ts);
    */
    
    //EULER BACKWARD
    float M1_q0=1+(ki*Ts)+(kd/Ts);
    float M1_q1=-(1+(2*kd/Ts));
    float M1_q2=(kd/Ts);
    
    
    cv=cv_1+kp*(M1_q0*e+M1_q1*error_1+M1_q2*error_2);
