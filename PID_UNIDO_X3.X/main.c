...
void main(void)
{
    ...
    {   
        ...
        if(enable_ident==1)
        {
            ...
            PID3(1500,'I');
            //UART1_Write2(Vel3);
            UART1_Write3(Vel1,Vel2,Vel3,1500,1500,1500);
            __delay_ms(15);
        }
        else
        {
...
void PID1(unsigned int sp,char rot)
{
    ...
    else
    {
        ...
        cv1=cv1_1+M1_q0*e+M1_q1*error1_1+M1_q2*error1_2;
        if(cv1>=10000.0)
        {
            cv1=10000.0;
        }
...
void PID2(unsigned int sp,char rot)
{
    ...
    else
    {
        ...
        cv2=cv2_1+M2_q0*e+M2_q1*error2_1+M2_q2*error2_2;
        if(cv2>=10000.0)
        {
            cv2=10000.0;
        }
...
void PID3(unsigned int sp,char rot)
{
    ...
    else
    {
        ...
        cv3=cv3_1+M3_q0*e+M3_q1*error3_1+M3_q2*error3_2;
        if(cv3>=10000.0)
        {
            cv3=10000.0;
        }
...
{
    //EULER BACKWARD M1
    M1_q0=kp1+(ki1*Ts)+(kd1/Ts);
    M1_q1=-(kp1+(2*kd1/Ts));
    M1_q2=(kd1/Ts);
    //EULER BACKWARD M2
    M2_q0=kp2+(ki2*Ts)+(kd2/Ts);
    M2_q1=-(kp2+(2*kd2/Ts));
    M2_q2=(kd2/Ts);
    //EULER BACKWARD M3
    M3_q0=kp3+(ki3*Ts)+(kd3/Ts);
    M3_q1=-(kp3+(2*kd3/Ts));
    M3_q2=(kd3/Ts);
}