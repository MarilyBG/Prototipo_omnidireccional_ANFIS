...
void UART1_Write4(unsigned int cont);
...
float W1_CI(float Vx_,float Vy_,float Wp_)
{
    ...
    OUT=OUT*1650/3.141592; //convertir de rad/s -> pps*10
    return OUT;
}
float W2_CI(float Vx_,float Vy_,float Wp_)
{
    ...
    OUT=OUT*1650/3.141592;//1122/3.141592; //convertir de rad/s -> pps*10
    return OUT;
}
float W3_CI(float Vx_,float Vy_,float Wp_)
{
...
float W1_fuzzy(float Vx_,float Vy_,float Wp_)
{
    ...
    //funcion de salida
    float Pert_Vec[]={-2445,-1382,-318.2,-2445,-1382,-318.2,-2445,-1382,-318.2,-1020,43.82,1107,-1020,43.82,1107,-1020,43.82,1107,405.9,1469,2533,405.9,1469,2533,405.9,1469,2533};
    //Fuzificacion 
    Vxb=fuzzy(Vx_,-242.3, -120.1, 1.988);
    Vxm=fuzzy(Vx_,-120.1, 1.988, 124.1);
    VxA=fuzzy(Vx_,1.988, 124.1, 246.3);
    
    Vyb=fuzzy(Vy_,-220.2, -109.9, 0.5094);
    Vym=fuzzy(Vy_,-109.9, 0.5094, 110.9);
    VyA=fuzzy(Vy_,0.5094, 110.9, 221.2);
    
    Wpb=fuzzy(Wp_,-30.94, -15.32, 0.3029);
    Wpm=fuzzy(Wp_,-15.32, 0.3029, 15.92);
    WpA=fuzzy(Wp_,0.3029, 15.92, 31.54);
...
float W2_fuzzy(float Vx_,float Vy_,float Wp_)
{
    ...
    float Num=0.0, Val_min_pert=0.0, OUT=0.0;
    //funcion de salida
    float Pert_Vec[]={-1452,-388.7,674.7,-336.5,726.9,1790,779.1,1842,2906,-2165,-1101,-38.01,-1049,14.17,1078,66.36,1130,2193,-2878,-1814,-750.7,-1762,-698.6,364.8,-646.4,417,1480};
    //Fuzificacion 
    Vxb=fuzzy(Vx_,-242.3, -120.1, 1.988);
    Vxm=fuzzy(Vx_,-120.1, 1.988, 124.1);
    VxA=fuzzy(Vx_,1.988, 124.1, 246.3);
...
float W3_fuzzy(float Vx_,float Vy_,float Wp_)
{
    ...
    float Num=0.0, Val_min_pert=0.0, OUT=0.0;
    //funcion de salida
    float Pert_Vec[]={768.8,1832,2896,-346.8,716.6,1780,-1462,-399,664.4,56.06,1119,2183,-1060,3.875,1067,-2175,-1112,-48.31,-656.7,406.7,1470,-1772,-708.8,354.5,-2888,-1824,-761};
    //Fuzificacion 
    Vxb=fuzzy(Vx_,-242.3, -120.1, 1.988);
    Vxm=fuzzy(Vx_,-120.1, 1.988, 124.1);
    VxA=fuzzy(Vx_,1.988, 124.1, 246.3);
